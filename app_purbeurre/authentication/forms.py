...
class SignUpForm(UserCreationForm):
    ...
    class Meta:
        """Define all the informations given to the user while he's registrating"""
        model = User
        fields = ('email', 'first_name', 'last_name', 'password1', 'password2', )

    email = forms.EmailField(label='Votre email', widget=EmailInput(attrs={'class': 'form-control'}),
                    error_messages={'required': 'Email requis',
                                    'invalid': 'Email non valide',
                                    'unique': 'Email déjà utilisé'})
    
    first_name = forms.CharField(required=False, label="Votre prénom (facultatif)", widget=TextInput(attrs={'class': 'form-control'}))
